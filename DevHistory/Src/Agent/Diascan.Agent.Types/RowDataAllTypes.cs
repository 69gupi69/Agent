namespace Diascan.Agent.Types
{
    public class RowDataAllTypes
    {
        ...
        public int? SensorNumberBlock { get; set; } // Номер датчика
        ...
        public double? AreaContiguous { get; set; } // Площадь ПДИ смежного датчика
        public enSensorRangeType RangeType { get; set; } // (Отказ)/(ВГС\загрязнение\повреждение)
        public enPipeType PipeType { get; set; } // (КПП СОД)/(Линейная часть)
    }
}
