...
namespace Diascan.Agent.AnalysisManager
{
    public class AnalysisManager
    {
        ...
        public AnalysisManager(Action<Calculation> updateAction, Action<object> loggerInfoAction)
        {
        ...
        private AnalysisType GetAnalysisType(List<DiagData> items, double diameter,DataType dataType, enCdmDirectionName dataTypeKey, double sumAreaLdi)
        {
        ...
        public AnalysisCalculation DoAnalysis(Calculation calculation)
        {
            ...
            foreach (var dataType in dataTypes)
            {
                ...
                if (dataType.Key == enCdmDirectionName.None)
                {
                    foreach (var noneType in dataType.Value)
                    {
                        ...
                        var areaContiguous = CalcAreasСontiguous(noneType, calculation.DataOutput.Diameter, out var sensorsСontiguous);
                ...
                else
                {
                    ...
                    {
                        var cdpaDiagDataList = new List<DiagData>();
                    ...
                    {
                        var cdmDiagDataList = new List<DiagData>();
        ...
        /// <returns></returns>
        public static double CalcAreasСontiguous(DiagData diagData, double diameter, out Dictionary<int, List<RowDataAllTypes>> sensorsСontiguous)
        {
        ...
        private static void СreatingListSensorBlocks(DiagData diagData, List<int> sensors, out Dictionary<int, List<SensorRange>> haltingSensorsBlocks, out List<int> sensorsBlocks)
        {
        ...
        /// <returns></returns>
        private List<RowDataAllTypes> FillFieldsTableByPdi(DiagData diagData, List<DiagData> diagDataList, out bool doubleAngle)
        {
        ...
        private void CommonParams(RowDataAllTypes rowData, DiagData diagData, SensorRange distRange, bool doubleAngleFlag)
        {
        ...
        private void CheckRestart(AnalysisCalculation analysisCalculation, Calculation calculation)
        {
            ...
            foreach (var dataType in dataTypes)
            {
                ...
                else
                {
                    ...
                    // линейная часть ПДИ
                    var areaLinearPDI = SearchAreaLinearPDI(calculation.DiagDataList.Where(q => q.DataType == DataType.Cd360 || q.DataType == DataType.Cd360)
                                                                                    ?.FirstOrDefault().SensorCount, calculation.DataOutput,
                                                            analysisCalculation.AnalisysTypeCollection.Where(q => q.DataType == DataType.Cd360 || q.DataType == DataType.Cd360 && q.CdmType == dataType.Key)
                                                                                                      ?.FirstOrDefault());
            ...
            //  3. Для ВИП УСК.04 количество смежных сбойных датчиков 2 и более протяженностью от 1 км
        ...
        public void NearSensorsMfl(ReferenceInputData dataOutput, IEnumerable<DiagData> diagDataList, List<RestartCriterion> restartCriterion, int nearSensorBlocks)
        {
        ...
        public void NearSensors(ReferenceInputData dataOutput, IEnumerable<DiagData> diagDataList, List<RestartCriterion> restartCriterion, int nearSensorCount)
        {
            foreach (var diagData in diagDataList)
            {
        ...
        private void CheckRestartLengthDifference(AnalysisCalculation analysisCalculation, Calculation calculation)
        {
            ...
            // разница длины (%)
            var lengthDifferencePercent = (calcDataOutput.PlotLengthTechSpec - distLength) * 100 / distLength;
            // разница длины (м)
            var lengthDifference = calcDataOutput.PlotLengthTechSpec - distLength;

            if (calcDataOutput.PlotLengthTechSpec <= 1000d && !calcDataOutput.GateValve)
            {
