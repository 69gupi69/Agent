...
namespace Diascan.Agent.CalcDiagDataLossTask
{
    public class DiagDataLossTask
    {
        ...
        public bool Calc(Action<Calculation> updateAction, Action<object> loggerInfo, Calculation calculation, DiagData diagData, CancellationTokenSource cts)
        {
                ...
                while (distEnd < diagData.MaxDistance - diagParams.IgnoreAreasCount * diagParams.Distance)
                {
                    ...
                    switch (diagData.DataType)
                    {
                        ...
                        case DataType.TfiT41:
                            FindTfiHaltingSensors(diagData, diagnosticData, diagParams, dist, distEnd);
                            break;
                        case DataType.MflT1:
                        ...
                        case DataType.MflT34:
                            ...
                            break;
                        case DataType.Ema:
                            FindEmaHaltingSensors(diagData, diagnosticData, diagParams, dist, distEnd);
                            break;
                    }
        ...
        private void FindCDpaHaltingSensors(CDpaDiagData diagData, DiagnosticData diagnosticData, DiagDataParams diagParams, Action<object> loggerInfo, double distStart, double distStop)
        {
            ...
            catch (Exception e)
            {
                ...
                if (e.StackTrace.Contains("Diascan.Utils.DataBuffers.UnmanagedMemoryHelper.VirtualQuery(UIntPtr address)"))
                    throw new Exception("restart calculation", e);
                else
        ...
        private DataHandle<float> GetDataHandle(DiagnosticData diagnosticData, DataType dataType, double distStart, double distStop)
        {
            try
            {
                switch (dataType)
                {
                    ...
                    case DataType.Wm:
                        return diagnosticData.GetWmWtData(distStart, distStop);
                    default:
                        return null;
                }
            }
        ...
        private unsafe void FindEmaHaltingSensors(DiagData diagData, DiagnosticData diagnosticData, DiagDataParams diagParams, double distStart, double distStop)
        {
        ...
        private unsafe List<Range<int>> CalcEmaHaltingSensors(EmaSensorItem* sensorItemPtr, EmaSensorItem* nextSensorItemPtr, int scanCount)
        {
            const int zeroDataScanCountThr = 30;
            var result = new List<Range<int>>();

            var zeroItemScanBeg = -1;
            var zeroItemScanCount = 0;

            for (var scanIndex = 0; scanIndex < scanCount; scanIndex++)
            {
                if (CheckNullSensorItem(sensorItemPtr) && CheckNullSensorItem(nextSensorItemPtr)) // <-
                {
                    zeroItemScanCount++;

                    if (zeroItemScanBeg < 0)
                        zeroItemScanBeg = scanIndex;

                    if (zeroItemScanCount >= zeroDataScanCountThr)
                    {
                        if (scanIndex + 1 == scanCount)
                            result.Add(new Range<int>(zeroItemScanBeg, zeroItemScanBeg + zeroItemScanCount));
                    }
                }
                else if (zeroItemScanCount >= zeroDataScanCountThr)
                {
                    result.Add(new Range<int>(zeroItemScanBeg, zeroItemScanBeg + zeroItemScanCount));
        ...
        private void CombineHaltingSensors(DiagData diagData, DiagDataParams diagParams, List<int> sensors, double distStart, double distStop, enSensorRangeType rangeType = enSensorRangeType.Other)
        {
        ...
        public void CheckCdTail(Calculation calculation, DiagData diagData, CancellationTokenSource cts, Action<Calculation> updateAction, Action<object> loggerInfo)
        {
            using (var diagnosticData = new DiagnosticData())
            {
                try
                ...
                foreach (var distRange in distRanges)
                {
                    ...
                    while (!MathHelper.TestDoubleEquals(distRange.End, distStart))
                    {
                        ...
                        calculation.ProgressCdlTail = $"{Math.Round(progressPercent, 2)}%";
                        updateAction?.Invoke(calculation);
                        distStart = distStop;
        ...
        public void CalculateNavigation(Calculation calculation, Action<object> loggerInfo)
        {
        ...
        //  BEP
        private void CalculateBepNavigation(DiagnosticData diagnosticData, Calculation calculation, NavCalcParams constNavCalcParams)
        {
        ...
        private unsafe void CalculateParametersInRun(DiagnosticData diagnosticData, Calculation calculation, Range<int> rangeNav)
        {
            var bins = false;
