...
namespace Diascan.Agent.ClientApp
{
    public class DistanceAlferov
    {
        ...
        public void SetData(List<Tuple<string, string, Guid>> datas)
        {
            ...
            foreach (var data in datas)
            {
                ...
                {
                    languageSet.Rus.Words = data.Item1.Split(new char[] { '"', '-', ',', '.', ' ', '/', '\\', '(', ')', '!', '№', '%' }, StringSplitOptions.RemoveEmptyEntries).Select(w => new Word()
                    {
                        Text = w.ToLower(),
                ...
                {
                    languageSet.Eng.Words = data.Item2.Split(new char[] { '"', '-', ',', '.', ' ', '/', '\\', '(', ')', '!', '№', '%' }, StringSplitOptions.RemoveEmptyEntries).Select(w => new Word()
                    {
                        Text = w.ToLower(),
                        Codes = GetKeyCodes(codeKeys, w)
                    }).ToList();
                }
                languageSet.IdGuid = data.Item3;
                this.Samples.Add(languageSet);
            }
        }
        public List<Tuple<string, string, Guid, double, int>> Search(string targetStr)
        {
            var codeKeys = CodeKeysRus.Concat(CodeKeysEng).ToList();
            ...
            var translationSearchObj = new AnalizeObject();
            if (targetStr.Length > 0)
            {
                translationSearchObj.Words = targetStr.Split(new char[] { '"', '-', ',', '.', ' ', '/', '\\', '(', ')', '!', '№', '%' }, StringSplitOptions.RemoveEmptyEntries).Select(w => {
            ...
            var result = new List<Tuple<string, string, Guid, double, int>>();
            foreach (var sampl in Samples)
            {
                ...
                result.Add(new Tuple<string, string, Guid, double, int>(sampl.Rus.Origianl, sampl.Eng.Origianl, sampl.IdGuid, cost, languageType));
            }
            return result;
        }
