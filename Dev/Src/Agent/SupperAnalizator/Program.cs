...
namespace SupperAnalizator
{
    public class Deserialize
    {
        ...
        public void GetDeserialize()
        {
            ...
            {
                var сommand = new NpgsqlCommand($"SELECT * FROM data.\"Sessions\"", connection);
                connection.Open();
                using (var reader = сommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        try
                        {
                            ...
                            var calculation = JsonConvert.DeserializeObject<Session>(text, settings);
                            accessManager.DiascanAgentAccess.SessionAccess.Insert(calculation);
                        }
                        catch (Exception e)
                        {
                            Console.WriteLine(e);
