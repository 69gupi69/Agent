...
namespace Diascan.Agent.AnalysisManager
{
    public class ExportToExcel
    {
        ...
        public byte[] CreateExcelFile(Dictionary<Calculation, AnalysisCalculation> dictionaryAnalysisCalculation, List<Calculation> calculation, out bool result)
        {
            result = false;
            ...
            foreach (var analysisCalculation in dictionaryAnalysisCalculation)
            {
                receptionChamber = analysisCalculation.Key.DataOutput.ReceptionChamber;
                triggerChamber = analysisCalculation.Key.DataOutput.TriggerChamber;
                //  первый лист Сводная
                //  "шапка" таблицы
                loggerInfoAction?.Invoke($"первый лист \n \"шапка\" таблицы");
                CreateHeaderSummaryTable(worksheetNumber, inspectionCount);
                //  отрисовка и заполние полей таблицы
                loggerInfoAction?.Invoke($"отрисовка и заполние полей таблицы");
                var rowIndex = CreateRowSummaryTable(analysisCalculation.Value, worksheetNumber);
                CreateCommonLinesSummaryTable(analysisCalculation.Key, rowIndex, worksheetNumber);
                //  таблица перезапуска ВИП
                loggerInfoAction?.Invoke($"таблица перезапуска ВИП");
                RestartVip(analysisCalculation.Value.RestartReport, worksheetNumber, rowIndex);

                //  таблицы с ПДИ
                foreach (var analysisType in analysisCalculation.Value.AnalisysTypeCollection)
                {
                    workBook.Worksheets.Add(analysisType.CdmType == enCdmDirectionName.None
                        ? $"Инсп.{inspectionCount} ПДИ по датчикам {analysisType.DataType.ToString().ToUpper()}"
                        : $"Инсп.{inspectionCount} ПДИ по датчикам {analysisType.CdmType.ToString().ToUpper()}");
                    worksheetNumber++;
                    CreateTableHeaderCalculationPdi(analysisCalculation.Key.DataOutput, analysisType.DataType);
                ...
                //  ПДИ по скорости
                loggerInfoAction?.Invoke($@"Инсп.{inspectionCount} ПДИ по скорости");
                workBook.Worksheets.Add();
                worksheetNumber++;
                workBook.Worksheets[worksheetNumber].Name = $@"Инсп.{inspectionCount} ПДИ по скорости";
                CreateTableBySpeed(analysisCalculation.Value, analysisCalculation.Key);

                //  ПДИ рамки
                loggerInfoAction?.Invoke($@"Инсп.{inspectionCount} ПДИ рамки");
                if (analysisCalculation.Key.CdChange)
                {
                    workBook.Worksheets.Add();
                    worksheetNumber++;
                    workBook.Worksheets[worksheetNumber].Name = $@"Инсп.{inspectionCount} Определение типа CD";
                    CreateHeaderTableByFrames(analysisCalculation.Key.DataOutput);
                    CreateTableByFrames(analysisCalculation.Key);
                }

                //  Навигация
                loggerInfoAction?.Invoke($@"Инсп.{inspectionCount} Навигация");
                if (analysisCalculation.Key.NavigationInfo.State.HasFlag(NavigationStateTypes.CalcNavigation))
                {
                    workBook.Worksheets.Add();
                    worksheetNumber++;
                    workBook.Worksheets[worksheetNumber].Name = $@"Инсп.{inspectionCount} Контроль качества НД";
                    CreateHeaderNavTable();
                    CreateBodyNavTable(analysisCalculation.Key);
                }

                // Просечки
                loggerInfoAction?.Invoke($@"Просечки");
                if (analysisCalculation.Key.DataOutput.Notch)
                {
                    foreach (var diagData in analysisCalculation.Key.DiagDataList)
                    {
                        if (diagData.DataType.IsNotchDataType())
                        {
                            workBook.Worksheets.Add();
                            worksheetNumber++;
                            workBook.Worksheets[worksheetNumber].Name = $@"Просечки по данным {diagData.DataType.ToString().ToUpper()}";
                            CreateTableHeaderNotch(analysisCalculation.Key.DataOutput);
                            //  отрисовка заголовка таблицы и самой таблицы датчиков
                            loggerInfoAction?.Invoke($"отрисовка заголовка таблицы и самой таблицы датчиков {diagData.DataType.ToString().ToUpper()}");
                            CreateTableBySensorsScansNotch((MpmDiagData)diagData);
                        }
                    }
                }
        ...
        private void CreateHeaderSummaryTable(int worksheetNumber,int inspectionCount)
        {
            workBook.Worksheets.Add($"Сводная_инспекция{inspectionCount}");
        ...
        private void CreateCommonLinesSummaryTable(Calculation calculation, int rowIndex, int worksheetNumber)
        {
            ...
            TableDesign(worksheetNumber, rowIndex, 10, firstRowIndex);
        ...
        private void CreateTableHeaderCalculationPdi(ReferenceInputData dataOutput, DataType dataType = DataType.None)
        {
            ...
            workSheet.Cells[row, column + 3].Value = dataOutput.DateWorkItem.ToShortDateString();
        ...
        private void CreateTableHeaderNotch(ReferenceInputData dataOutput)
        {
            var workSheet = workBook.Worksheets[worksheetNumber];
            workSheet.Cells[0, 0].Value = "№ п/п";
            workSheet.Cells[0, 1].Value = "Результаты автоматического расчета потерь диагностической информации по участку \n" + dataOutput.Route.Value;
            ...
            workSheet.Cells[row + 1, column + 2].Value = "Протяженность по ВИП, м: ";
            workSheet.Cells[row + 1, column + 3].Value = Math.Round(distLenght, 3);
            workSheet.Cells[row + 2, column + 2].Value = "Площадь обследования, м\u00B2: ";
            workSheet.Cells[row + 2, column + 3].Value = Math.Round(Math.PI * dataOutput.Diameter / 1000 * distLenght, 3);
            workSheet.Cells[row + 3, column + 2].Value = "Дефектоскоп №: ";
            workSheet.Cells[row + 3, column + 3].Value = dataOutput.FlawDetector;
            workSheet.Cells[row + 4, column + 2].Value = "Ответственный за пропуск: ";
            workSheet.Cells[row + 4, column + 3].Value = dataOutput.ResponsibleWorkItem;
            ...
            TableDesign(worksheetNumber, 2, 5, 0, 0, Styles.DifferentFontBorderType(12));
            TableDesign(worksheetNumber, 5, 1, 2, 0, Styles.BaseDoubleCellBorderType(12));

            TableDesign(worksheetNumber, 5, 1, row, column, Styles.BaseDoubleCellBorderType(12));
            TableDesign(worksheetNumber, 5, 1, row, column + 1, Styles.DifferentFontBorderType(12));
            TableDesign(worksheetNumber, 5, 1, row, column + 2, Styles.BaseDoubleCellBorderType(12));
            TableDesign(worksheetNumber, 5, 1, row, column + 3, Styles.DifferentFontBorderType(12));
        }

        private void FillNotchTableBody(Worksheet worksheet, List<Tuple<int, int, double>> notches, int rowOffset, ref int lastNotchIndex)
        {
            var sheetNotchIndex = 0;
            for (var notchIndex = lastNotchIndex; notchIndex < notches.Count; notchIndex++)
            {
                var notchItem = notches[notchIndex];

                worksheet.Cells[rowOffset + sheetNotchIndex, 0].Value = notchIndex + 1;
                worksheet.Cells[rowOffset + sheetNotchIndex, 1].Value = notchItem.Item1 + 1; // считаем не от 0, а от 1 
                worksheet.Cells[rowOffset + sheetNotchIndex, 2].Value = notchItem.Item2;
                worksheet.Cells[rowOffset + sheetNotchIndex, 3].Value = Math.Round(notchItem.Item3, 3);

                sheetNotchIndex++;

                if (rowOffset + sheetNotchIndex >= MaxRowSheet)
                {
                    lastNotchIndex = notchIndex + 1;
                    break;
                }
            }

            TableDesign(worksheetNumber, sheetNotchIndex, 4, rowOffset);
            worksheet.AutoFitColumns(0, 5);
        }

        private void CreateTableBySensorsScansNotch(MpmDiagData diagData)
        {
            var recordsCount = diagData.NotchsList.Sum(z => z.Scans.Count);
            var countSheet = Math.Ceiling((double)recordsCount / MaxRowSheet);  //  количество листов с ПДИ для одного типа данных
            var headerOffset = 7;

            if (diagData.NotchsList.Count == 0)
            {
                var workSheet = workBook.Worksheets[worksheetNumber];
                workSheet.Cells[headerOffset, 0].Value = "Просечки не обнаружены";
                workSheet.Cells.Merge(headerOffset, 0, 1, 4);
                TableDesign(worksheetNumber, 1, 4, headerOffset, 0, Styles.DifferentFontStyle());
                return;
            }

            var notches = new List<Tuple<int, int, double>>(recordsCount);
            foreach (var notch in diagData.NotchsList)
                foreach (var scan in notch.Scans)
                    notches.Add(new Tuple<int, int, double>(notch.SensorIndex, scan.ScansIndx, scan.Distance));
            ...
            for (var sheetIndex = 0; sheetIndex < countSheet; sheetIndex++)
            {
                ...
                workSheet.Cells[headerOffset, 0].Value = $"ТИП ДАТЧИКОВ ({diagData.DataType.ToString().ToUpper()})";
                workSheet.Cells.Merge(headerOffset, 0, 1, 4);

                workSheet.Cells[headerOffset + 1, 0].Value = "№ п/п";
                workSheet.Cells[headerOffset + 1, 1].Value = "№ датчика";

                workSheet.Cells[headerOffset + 1, 2].Value = "№ Скана";
                workSheet.Cells[headerOffset + 1, 3].Value = "Дистанция просечки, м";

                for (var j = 0; j < 4; j++)
                    workSheet.Cells[headerOffset + 2, j].Value = j + 1;

                TableDesign(worksheetNumber, 1, 4, headerOffset, 0, Styles.DifferentFontBorderType(16));
                TableDesign(worksheetNumber, 2, 4, headerOffset + 1, 0, Styles.DifferentFontBorderType(11));
                TableDesign(worksheetNumber, 5, 1, headerOffset + 1, 0, Styles.BaseDoubleCellBorderType(12));
                workSheet.AutoFitColumns(1, 5);

                FillNotchTableBody(workSheet, notches, headerOffset + 3, ref notchIndex);

                if (sheetIndex + 1 < countSheet)
                {
                    workBook.Worksheets.Add($@"Просечки по данным {diagData.DataType.ToString().ToUpper()}_{sheetIndex + 1}");
                    workSheet.AutoFitColumns(1, 5);
                    worksheetNumber++;
                    headerOffset = 0;
                }
        ...
        private void CreateHeaderTableByFrames(ReferenceInputData referenceInputData)
        {
            ...
            workSheet.Cells[firstRowIndex, 3].Value = referenceInputData.DateWorkItem.ToShortDateString();
        ...
        private void CreateTableBySensors(AnalysisType analysisType)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (analysisType.DoubleAngle)
                {
                    ...
                    if (analysisType.DataType == DataType.Cd360)
                    {
