...
namespace Diascan.Agent.TaskManager
{
    ...
    {
        public static Dictionary<enCdmDirectionName, List<IDiagData>> GroupDiagDataByType(List<IDiagData> diagDataList)
        {
            var result = new Dictionary<enCdmDirectionName, List<IDiagData>>();
            foreach (var diagData in diagDataList)
            {
                ...
                if (diagData is CdmDiagData cdmDiagData)
                {
                    ...
                    if (!result.ContainsKey(dirName))
                        result.Add(dirName, new List<IDiagData>());
                ...
                else
                {
                    if (!result.ContainsKey(enCdmDirectionName.None))
                        result.Add(enCdmDirectionName.None, new List<IDiagData>());
