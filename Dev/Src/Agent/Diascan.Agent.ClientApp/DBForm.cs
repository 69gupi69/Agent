...
namespace Diascan.Agent.ClientApp
{
    public partial class DBForm : Form
    {
        ...
        private async void DBForm_Shown(object sender, EventArgs e)
        {
            ...
            headersSession = await SharingEvents.SharingEvents.OnFormLoad();
            dgvDataBase.DataSource = headersSession;
            headersSession.ListChanged += DataSourceChanged;
            UpdateButtons();
        ...
        {
            if (headersSession == null || headersSession.Count <= 0)
            {
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                var headerCalculation = (HeaderSession)row.DataBoundItem;
        ...
        private void ShowInputForm(bool local)
        {
            using (var openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.Cancel)
                    return;
                var datasetLocation = new List<DataLocation>();
                var referenceInputData = new ReferenceInputData();
                try
                {
                    Form form;
                    if (local)
                        form = new CiwInput(referenceInputData, openFileDialog.FileName, datasetLocation);
                    else
                        form = new DataInput(referenceInputData, openFileDialog.FileName, datasetLocation);
                    ...
                    else
                    {
                        ...
                        if (referenceInputData.WorkItemName != null)
                            SharingEvents.SharingEvents.OnSentPdi(openFileDialog.FileName, referenceInputData,  datasetLocation);
                    }
                }
                catch (Exception ex)
                {
                    if(ex.TargetSite.DeclaringType.Name.Contains("CiwInput"))
                        MessageBox.Show("Ошибка добавления СМР расчета:\n" + ex.Message);
                    else
                        MessageBox.Show("Ошибка добавления расчета:\n" + ex.Message);
                }
            }
        }
        ...
        private void toolUpdatingDirectories_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SharingEvents.SharingEvents.OnDirectoryCarriersUpdate();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Поиск просечек на датчики T1, T11, T4, T41
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnNotch_Click(object sender, EventArgs e)
        {
            using (var openFileDialog = new OpenFileDialog())
            {
                openFileDialog.Filter = "Text files(*.omni)|*.omni";
                ...
                try
                {
                    using (var form = new DataInput(referenceInputData, openFileDialog.FileName, datasetLocation))
                    {
                        if (form.DialogResult == DialogResult.Cancel)
                        {
                            form.Dispose();
